...
@Slf4j
@Component
public class GameFiDispatcher {

    @Resource
    private IGameFiProvider<?>[] providers;

    private final Map<String, IGameFiProvider<?>> providerMap = new HashMap<>();

    @PostConstruct
    public void initProvider() {
        for (IGameFiProvider<?> provider : providers) {
            Type[] types = provider.getClass().getGenericInterfaces();
            Class<?>[] typeArguments = Arrays.stream(types)
                    .filter(type -> {
                        if (type instanceof ParameterizedType) {
                            ParameterizedType pt = (ParameterizedType) type;
                            if (pt.getRawType().equals(IGameFiProvider.class)) {
                                return true;
                            }
                        }
                        return false;
    ...
    public <T> void dispatch(T entity) {
        ...
        IGameFiProvider<T> provider = (IGameFiProvider<T>) providerMap.get(entity.getClass().getName());
